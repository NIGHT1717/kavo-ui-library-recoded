...
function Kavo.CreateLib(kavName, themeList)
    ...
    local tabFrames = Instance.new("ScrollingFrame")  -- Modified: Changed to ScrollingFrame for horizontal tab scrolling
    local tabListing = Instance.new("UIListLayout")  -- Horizontal layout for tabs
    ...
    -- Modified: tabFrames as ScrollingFrame for horizontal tab scrolling
    ...
    tabFrames.ScrollBarThickness = 6  -- Thin scrollbar
    tabFrames.ScrollingDirection = Enum.ScrollingDirection.X  -- Horizontal scroll
    tabFrames.CanvasSize = UDim2.new(0, 0, 0, 0)  -- Initial canvas
    tabFrames.ScrollBarImageColor3 = themeList.SchemeColor  -- Theme color for scrollbar
    tabFrames.MidImage = "rbxassetid://3570695787"
    tabFrames.MidImageConstraint = Enum.ScaleType.Slice
    tabFrames.MidImageFillDirection = Enum.FillDirection.Horizontal
    tabFrames.MidImageRepeat = Enum.RepeatBehavior.NoRepeat
    tabFrames.TopImage = "rbxassetid://3570695787"
    tabFrames.BottomImage = "rbxassetid://3570695787"

    tabListing.Name = "tabListing"
    tabListing.Parent = tabFrames
    tabListing.SortOrder = Enum.SortOrder.LayoutOrder
    tabListing.FillDirection = Enum.FillDirection.Horizontal  -- Horizontal layout for tabs
    tabListing.Padding = UDim.new(0, 5)  -- Padding between tabs
    ...
    -- Function to update tab canvas size
    local function updateTabCanvas()
        local totalWidth = 0
        local layout = tabFrames:FindFirstChild("UIListLayout")
        if layout then
            for _, child in pairs(tabFrames:GetChildren()) do
                if child:IsA("TextButton") then
                    totalWidth = totalWidth + child.AbsoluteSize.X + layout.Padding.Offset
                end
            end
            totalWidth = math.max(totalWidth, tabFrames.AbsoluteSize.X)
            tabFrames.CanvasSize = UDim2.new(0, totalWidth, 0, 0)
        end
    end
    ...
    function Tabs:NewTab(tabName)
        ...
        -- Update tab canvas after adding tab (with small delay for rendering)
        task.wait(0.1)
        updateTabCanvas()

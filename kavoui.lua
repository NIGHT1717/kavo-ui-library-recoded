...
task.wait(0.1)  -- Короткая задержка для рендера
updateTabCanvas()
...
local tabFrames = Instance.new("ScrollingFrame")  -- Изменено с Frame на ScrollingFrame
...
tabFrames.ScrollBarThickness = 6  -- Тонкий скролл-бар
tabFrames.ScrollingDirection = Enum.ScrollingDirection.X  -- Горизонтальный скролл
tabFrames.CanvasSize = UDim2.new(0, 0, 0, 0)  -- Начальный канвас
tabFrames.ScrollBarImageColor3 = themeList.SchemeColor  -- Цвет по теме
...
tabListing.FillDirection = Enum.FillDirection.Horizontal  -- Горизонтальное расположение
tabListing.Padding = UDim.new(0, 5)  -- Отступы между табами
...
-- Функция для обновления канваса табов (добавь здесь)
